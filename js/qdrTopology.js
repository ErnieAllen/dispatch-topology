...
var QDR = (function (QDR) {
   ...
   */
  QDR.module.controller("QDR.TopologyController", [
    "$scope",
    "$rootScope",
    "QDRService",
    "$location",
    ...
    function ($scope, $rootScope, QDRService, $location, $timeout, $uibModal) {
      ...
      var doOperation = function (operation, callback, extraProps) {
        ...
        var props = {
          nodes: nodes,
          links: l,
          topology: $scope.mockTopologyDir,
          settings: settings,
        };
        if (extraProps) Object.assign(props, props, extraProps);
        QDRService.sendMethod(operation, props, function (response) {
          if (callback) {
            console.log(`called ${operation} and all I got was`);
      ...
      var switchTopology = function (topology) {
        ...
        QDRService.sendMethod("SWITCH", props, function (response) {
          if ($scope.mockTopologies.indexOf(topology) == -1) {
            $timeout(function () {
              $scope.mockTopologies.push(topology);
            });
          }
          nodes = [];
          links = [];
          var savedPositions = localStorage[topology]
            ? angular.fromJson(localStorage[topology])
            : undefined;
          for (var i = 0; i < response.nodes.length; ++i) {
            ...
            if (node["host"]) anode["host"] = node["host"];
            ...
            sections.forEach(function (section) {
              if (node[section + "s"]) {
                for (var key in node[section + "s"]) {
                  ...
                  if (
                    section === "connector" &&
                    key == settings.artemis_port &&
                    node["connectors"][settings.artemis_port + ""]["role"] ===
                      "route-container"
                  )
                    type = "artemis";
                  if (
                    section === "connector" &&
                    key == settings.qpid_port &&
                    node["connectors"][settings.qpid_port + ""]["role"] ===
                      "route-container"
                  )
                    type = "qpid";
      ...
      var delNode = function (node, skipinit) {
        ...
        if (node.nodeType !== "inter-router") {
          ...
          if (n) delete n[node.entity + "s"][node.entityKey];
        } else {
          var sub_nodes = [];
        ...
        while (i--) {
          if (
            links[i].source.name === node.name ||
            links[i].target.name === node.name
          ) {
            links.splice(i, 1);
          }
        }
      ...
      var genNodeToAdd = function (contextNode, type, entityKey) {
        ...
        var dir = "out";
        if (type === "listener") dir = "in";
        else if (type === "console" || type === "sslProfile") dir = "both";
        var name = contextNode.name + "." + (clientLen + 1);
        nodeType = "normal";
        var properties =
          type === "console" ? { console_identifier: "Dispatch console" } : {};
        if (type === "artemis") {
        ...
        var node = aNode(
          ...
          nodes.length,
          contextNode.x,
          contextNode.y - radius - radiusNormal,
          contextNode.id,
          false,
          properties
        );
      ...
      $scope.hasConsoleListener = function (node) {
        if (!node) {
          for (var i = 0; i < nodes.length; i++) {
            var n = nodes[i];
            var found = n.listeners
              ? Object.keys(n.listeners).some(function (key) {
                  return key == settings.http_port;
                })
              : false;
        ...
        return node.listeners
          ? Object.keys(node.listeners).some(function (key) {
              return key == settings.http_port;
            })
          : false;
      };
      ...
      $scope.getSectionList = function (node, section) {
        if (node && node[section + "s"]) {
          if (section === "listener")
            return node && node.listeners
              ? Object.keys(node.listeners).filter(function (key) {
                  return !node.listeners[key].http;
                })
              : [];
          else return Object.keys(node[section + "s"]);
        }
        return [];
      };
      ...
      $scope.addConsoleListener = function (node) {
        ...
        node.listeners[settings.http_port] = {
      ...
      $scope.addAnotherNode = function (calc, edge) {
        ...
        if (calc) {
          ...
          while (overlap) {
            ...
            for (var i = 0; i < nodes.length; i++) {
              if (
                Math.abs(nodes[i].x - x) < radiusNormal * 2 &&
      ...
      var maxNodeIndex = function () {
        ...
        nodes.forEach(function (node) {
          if (node.nodeType === "inter-router") {
            if (node.id > maxIndex) maxIndex = node.id;
          }
        });
        return maxIndex;
      };
      ...
      $scope.reverseLink = function () {
        if (!mousedown_link) return;
        var d = mousedown_link;
        for (var i = 0; i < links.length; i++) {
          if (
            links[i].source.index === d.source.index &&
            links[i].target.index === d.target.index
          ) {
      ...
      var findChildNode = function (entity, entityKey, name) {
        ...
        for (var i = 0; i < nodes.length; i++) {
          if (
            ...
            nodes[i].entityKey == entityKey &&
            nodes[i].routerId === name
          )
            return nodes[i];
        }
      };
      var findParentNode = function (node) {
        ...
        for (var i = 0; i < nodes.length; i++) {
          if (
            nodes[i][node.entity + "s"] &&
            node.entityKey in nodes[i][node.entity + "s"]
          )
            return nodes[i];
        }
      };
      ...
      $scope.setRouterHost = function (multi) {
        if (!$scope.selected_node) $scope.selected_node = firstSelectedNode();
      ...
      $scope.editThisSection = function (node) {
        var n = findParentNode(node);
        if (n) doEditDialog(n, node.entity, node.entityKey, false);
      };
      ...
      var getSizes = function () {
        ...
        if (width < 10) {
          QDR.log.info(
            "page width and height are abnormal w:" +
              width +
              " height:" +
              height
          );
      ...
      var forceScale = function (nodeCount, min, max) {
        ...
        var x = d3.scale.linear().domain([6, 200]).range([min, max]);
      ...
      var initLegend = function () {
        ...
        lsvg = d3.select("#svg_legend").append("svg").attr("id", "svglegend");
        lsvg = lsvg
          .append("svg:g")
          .attr(
            "transform",
      ...
      var initForce = function () {
        ...
        force = d3.layout
          ...
          .linkDistance(function (d) {
            return linkDistance(d, routerCount);
          })
          .charge(function (d) {
            return charge(d, routerCount);
          })
          .friction(0.1)
          .gravity(function (d) {
            return gravity(d, routerCount);
          })
          .on("tick", tick)
          .on("end", function () {
            savePositions();
      ...
      var appendTitle = function (g) {
        g.append("svg:title").text(function (d) {
          ...
          if (d.cdir === "in") return "Listener on port " + d.entityKey;
          if (d.cdir === "out")
            return "Connector to " + d.host + ":" + d.entityKey;
          if (d.cdir === "both") return "sslProfile";
          if (d.cls === "host") return "Host " + d.name;
      ...
      var appendContent = function (g) {
        ...
        g.append("svg:text")
          ...
          .attr("y", function (d) {
            var y = 7;
            if (QDRService.isArtemis(d)) y = 8;
            else if (QDRService.isQpid(d)) y = 9;
          ...
          .text(function (d) {
            if (QDRService.isConsole(d)) {
              ...
              return d.properties.product
                ...
                : "S";
            } else if (d.nodeType === "normal" && d.cdir === "in")
              // listener
              return "\uf2a0";
            // phone top
            else if (d.nodeType === "normal" && d.cdir === "out")
              // connector
              return "\uf2a0";
            // phone top (will be rotated)
            else if (d.nodeType === "normal" && d.cdir === "both")
              // not used
              return "\uf023";
            // icon-laptop for clients
            else if (d.nodeType === "host") return "";
      ...
      function restart(start) {
          ...
          .attr("marker-start", function (d) {
            var sel =
              d === selected_link
                ...
                ? "-small"
                : "";
            if (d.highlighted) sel = "-highlighted";
        ...
        appendCircle(g)
          ...
          .on("mousedown", function (d) {
            ...
            initial_mouse_down_position = d3
              .mouse(this.parentElement.parentElement.parentElement)
              .slice();
          })
          .on("mouseup", function (d) {
            ...
            if (
              Math.abs(cur_mouse[0] - initial_mouse_down_position[0]) > 4 ||
            ...
            if ($scope.selected_node && d !== $scope.selected_node) {
              ...
              if (
                nodes[source].nodeType === "edge" &&
                nodes[target].nodeType === "edge"
              )
                return;
              var curLinkCount = links.length;
              var newIndex = getLink(
                source,
                target,
                "in",
                "",
                genLinkName(d, $scope.selected_node)
              );
          ...
          .on("contextmenu", function (d) {
            ...
            var rm = relativeMouse();
            var menu =
              d.cls === "router" ? "action_menu" : "client_context_menu";
            if (isSelectedNode(d)) menu = "multiple_action_menu";
            d3.select("#" + menu)
              .style("left", rm.left + "px")
              .style("top", rm.top - rm.offset.top + "px")
              .style("display", "block");
        ...
        if (!mousedown_node || !$scope.selected_node) return;
      ...
      function updateLegend() {
        ...
        if (!svg.selectAll("circle.console").empty()) {
          legendNodes.push(
            aNode("Console", "", "normal", "console", 1, 0, 0, 0, false, {
        ...
        if (!svg.selectAll("circle.log").empty()) {
          legendNodes.push(
            aNode("Logs", "", "normal", "log", 6, 0, 0, 0, false, {})
          );
        }
        if (!svg.selectAll("circle.qpid-cpp").empty()) {
          legendNodes.push(
            genNodeToAdd(
              { key: "Qpid broker", name: "legend", x: 0, y: 0, id: "legend" },
              "qpid",
              ""
            )
          );
      ...
      function initHosts() {
        ...
        for (var i = 0; i < 10; i++) {
          if (sheet.insertRule) {
            ...
            sheet.insertRule(
              "circle.node.host" + i + " {fill: " + hostColors(i) + ";}",
              0
            );
            sheet.insertRule(
              "circle.node.host" +
                i +
                ".multi-selected {fill: url(#host_" +
                i +
                ");}",
              1
            );
            ...
            sheet.addRule(
              "circle.node.host" + i + ".multi-selected",
              "fill: url(#host_" + i + ")",
              1
            );
            sheet.addRule(
              "div.node.host" + i,
              "background-color: " + hostColors(i) + ";",
              2
            );
          }
        }
      ...
      function getHosts() {
        ...
        for (var i = 0; i < nodes.length; i++) {
          var node = nodes[i];
          if (node.host && node.cls === "router") hosts[node.host] = 1;
        }
        return Object.keys(hosts);
      }
      initHosts();
      function getHostIndex(h) {
        if (h in knownHosts) return knownHosts[h];
        ...
        for (var i = 0; i < 10; i++) {
          sel.classed("host" + i, function (d) {
            return (
              ...
              i === getHostIndex(d.host) &&
              (d.cls === "router" || d.cls === "host")
            );
          });
        }
      }
      ...
      function toggleSelectedNode(d) {
        if (d.name in $scope.selected_nodes)
          delete $scope.selected_nodes[d.name];
        else $scope.selected_nodes[d.name] = d;
        updateSelectedNodes();
      }
        ...
        $timeout(function () {
          d.result.then(function (result) {
            if (result) {
              ...
              if (multi) {
                for (var i = 0; i < nodes.length; i++) {
                  if (isSelectedNode(nodes[i])) setAHost(nodes[i]);
                }
              } else setAHost(node);
              resetCircleHosts();
              restart();
      ...
      function valFromMapArray(ar, key, val) {
        for (var i = 0; i < ar.length; i++) {
          if (ar[i][key] && ar[i][key] === val) return ar[i];
        }
        return undefined;
      }
            ...
            maxPort: function () {
              ...
              nodes.forEach(function (node) {
                if (node.entity === "listener" || node.entity === "connector") {
                  if (
                    node.entityKey !== "amqp" &&
                    node.entityKey != settings.artemis_port &&
                    node.entityKey != settings.qpid_port
                  )
            ...
            if (entity === "router") {
              ...
              if ("del" in result) {
                ...
                if (rVals) {
                  ...
                  if (entity === "sslProfile") return;
                  if (
                    context === "new" ||
                    context === "artemis" ||
                    context === "qpid"
                  ) {
                    if (context !== "new") entity = context;
  ...
  QDR.module.controller("QDR.SetRouterHostDialogController", function (
    $scope,
    $uibModalInstance,
    host
  ) {
  ...
  QDR.module.controller("QDR.ShowConfigDialogController", function (
    $scope,
    $uibModalInstance,
    config
  ) {
  ...
  QDR.module.controller("QDR.NewDialogController", function (
    $scope,
    $uibModalInstance,
    list
  ) {
  ...
  QDR.module.controller("QDR.SettingsDialogController", function (
    $scope,
    $uibModalInstance,
    settings
  ) {
    ...
    $scope.entity = {
      ...
      attributes: [
        {
          name: "baseName",
          humanName: "Starting router name",
          input: "input",
          type: "text",
          value: local_settings.baseName,
          required: true,
        },
        {
          name: "http",
          humanName: "Port for console listeners",
          input: "input",
          type: "text",
          value: local_settings.http_port,
          required: true,
        },
        {
          name: "normal_port",
          humanName: "Starting port for normal listeners/connectors",
          input: "input",
          type: "text",
          value: local_settings.normal_port,
          required: true,
        },
        {
          name: "internal_port",
          humanName: "Starting port for inter-router listeners/connectors",
          input: "input",
          type: "text",
          value: local_settings.internal_port,
          required: true,
        },
      ],
    };
  ...
  QDR.module.controller("QDR.DeployDialogController", function (
    $scope,
    $uibModalInstance,
    QDRService,
    $timeout,
    ...
    var pollTimer = null;
    function doPoll() {
      QDRService.sendMethod("DEPLOY-STATUS", { config: dir }, function (
        response
      ) {
        if (response[1] !== "DEPLOYING") {
          ...
          if (response[1] === "DONE") {
            ...
            if (http_host && http_port) {
              $scope.address = $sce.trustAsHtml(
                "http://" + http_host + ":" + http_port + "/#!/topology"
              );
            }
          } else {
        ...
        $timeout(function () {
          ...
          scrollToEnd();
          if ($scope.polling) pollTimer = setTimeout(doPoll, 1000);
        });
      });
    }
    $scope.deploy = function () {
