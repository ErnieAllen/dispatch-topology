...
var QDR = (function(QDR) {
  QDR.module.controller("QDR.NodeDialogController", function(
    $scope,
    $sce,
    $uibModalInstance,
    QDRService,
    ...
    if (!angular.isDefined(context)) $scope.context = "new";
    var newContext = {
      new: maxPort + 1,
      artemis: 61616,
      qpid: 5672,
    ...
    $scope.title =
      (context && context in newContext ? "Add " : "Edit ") +
      entityType +
      " section";
    if (context === "artemis") $scope.title += " to an Artemis broker";
    if (context === "qpid") $scope.title += " to a Qpid broker";
    var schema = QDRService.schema;
    var myEntities = [];
    myEntities.push(entityType);
    var readOnlyAttrs = {
      router: ["version", "id", "identity"],
      log: ["name", "identity"],
      address: ["identity"],
      linkRoute: ["identity"],
      sslProfile: ["identity"],
    ...
    var getEdivAttr = function(ediv, key) {
      for (var i = 0; i < ediv.attributes.length; i++) {
        if (ediv.attributes[i].name === key) return ediv.attributes[i];
      }
    };
    ...
    var entity = function(actualName, tabName, humanName, ent, icon, link) {
      ...
      var info = {
        ...
        attributes: $.map(ent.attributes, function(value, key) {
          ...
          if (
            readOnlyAttrs[tabName] &&
            readOnlyAttrs[tabName].indexOf(key) >= 0
          )
            return null;
          ...
          return {
            ...
            value: val,
            required:
              value.required || key === entityKey || key === "host"
                ? true
                : false,
            unique: value.unique
          };
    ...
    var stripAnnotations = function(entityName, ent, annotations) {
      if (ent.references) {
        ...
        ent.references.forEach(function(annoKey) {
          if (!annotations[annoKey]) annotations[annoKey] = {};
    ...
    myEntities.forEach(function(entityName) {
      ...
      if (entityName == "listener") hName = "Listener for clients";
      ...
      if (node[entityName + "s"] && context in node[entityName + "s"]) {
        ...
        ediv.attributes.forEach(function(attr) {
          if (attr["name"] in o) {
            ...
            if (Array.isArray(attr["rawtype"])) attr.selected = attr["value"];
          }
        });
      }
      ...
      if (ediv.actualName == "router") {
        ...
        if (node.nodeType === "edge")
          roleAttr.value = roleAttr.selected = "edge";
        else if (hasLinks && roleAttr) {
      ...
      if (ediv.actualName == "listener" && context === "new") {
        ...
        if (port) port.value = ++maxPort;
        var host = getEdivAttr(ediv, "host");
        if (host && node.host) host.value = node.host;
      }
      ...
      if (ediv.actualName == "log") {
        ...
        if (moduleAttr) moduleAttr.value = moduleAttr.selected = "DEFAULT";
        ...
        if (context === "new" && node.logs) {
          ...
          ediv.attributes.forEach(function(attr) {
            if (attr["name"] in log) {
              ...
              if (attr["name"] == "module") attr.selected = attr["value"];
            }
          });
        }
      }
      ...
      for (var i = 0, keys = entityKey.split("|"); i < keys.length; i++) {
        ...
        if (keyIndex > 0) allNames.move(keyIndex, 0);
      }
      ...
      ediv.attributes.sort(function(attr1, attr2) {
        return (
          allNames.indexOf(attr1["name"]) - allNames.indexOf(attr2["name"])
        );
      });
    ...
    $scope.isItDisabled = function(attribute) {
      if (
        requiredAttrs[entityType] &&
        requiredAttrs[entityType].indexOf(attribute.name) > -1
      ) {
        return requiredAttrs[entityType].some(function(attr) {
          return (
            attr !== attribute.name &&
            getEdivAttr($scope.entities[0], attr).value
          );
        });
      } else return false;
    };
    ...
    $scope.isItRequired = function(attribute) {
      if (
        ...
        return !s;
      } else return attribute.required;
    };
    ...
    $scope.setDescription = function(attr, e) {
      $scope.attributeDescription = $sce.trustAsHtml(
        attr.description +
          '<div class="attr-type">Type: ' +
          JSON.stringify(attr.rawtype, null, 1) +
          "</div>" +
          '<div class="attr-required">' +
          (attr.required ? "required" : "") +
          "</div>" +
          '<div class="attr-unique">' +
          (attr.unique ? "Must be unique" : "") +
          "</div>"
      );
    };
